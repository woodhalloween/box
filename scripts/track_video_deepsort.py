...
def main(args):
    ...
    if args.tracker == 'deepsort':
        ...
        raise ValueError(f'このスクリプトはDeepSORT専用です。指定されたトラッカー {args.tracker} はサポートされていません。')
            ...
            if len(boxes) > 0:
                ...
                for i in range(len(boxes)):
                    ...
                    if cls in args.classes:
                        detections_for_deepsort.append(([x1, y1, w, h], conf, str(cls))) # class_idは文字列にする必要あり
...
if __name__ == "__main__":
    ...
    parser.add_argument('--yolo-model', type=str, required=True, help='YOLOモデルパス (例: yolov8n.pt, yolo11n-pose.pt)')
    parser.add_argument('--tracker', type=str, default='deepsort', choices=['deepsort'], help='使用するトラッカー (このスクリプトではdeepsortのみ)')
    ...
    parser.add_argument('--half', action='store_true', help='YOLOで半精度浮動小数点数を使用 (DeepSORT自体には影響しない場合あり)')
