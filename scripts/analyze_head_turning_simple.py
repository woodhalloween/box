...
for i, event in events_data.iloc[:5].iterrows():
    ...
    event_data = skeleton_data[
        (skeleton_data["frame_idx"] >= min_frame) & (skeleton_data["frame_idx"] <= max_frame)
    ].copy()
    ...
    for lm in head_landmarks:
        x_col = f"landmark_{lm}_x"
        plt.plot(event_data["frame_idx"], event_data[x_col], label=f"Landmark {lm} X")
    ...
    for lm in head_landmarks:
        diff_col = f"landmark_{lm}_x_diff"
        plt.plot(
            event_data["frame_idx"].iloc[1:],
            event_data[diff_col].iloc[1:],
