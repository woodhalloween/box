...
def initialize_perf_log(enable_perf_log: bool, input_file: str, model_path: str):
    ...
    perf_columns = ['Frame', 'Time', 'Detection_Time_ms', 'Tracking_Time_ms', 'Stay_Check_Time_ms', 'Total_Time_ms',
                    'Objects_Detected', 'Objects_Tracked', 'FPS', 'Model', 'Tracker', 'Notes']
...
def process_frame_for_tracking(frame_rgb, model, tracker):
    ...
    return confirmed_tracks, detection_time_ms, tracking_time_ms, len(detections), len(confirmed_tracks)
    ...
    for track in tracks:
        ...
        if track_id not in stay_info:
            ...
            if stay_info[track_id]['stay_duration'] >= stay_threshold_sec and not stay_info[track_id]['notified']:
                notifications.append(f"通知: ID {track_id} が座標 ({int(center_x)}, {int(center_y)}) 付近に {stay_threshold_sec}秒 以上滞在中。")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--stay-threshold", type=float, default=6.0, help="滞在通知を行う時間の閾値 (秒, デフォルト: 4.0)")
