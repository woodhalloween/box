    ...
    def detect_pose(self, frame, frame_idx=0, display=True):
        ...
        if results.pose_landmarks:
            ...
            right_ear_x, right_ear_y = (
                int(right_ear.x * frame_width),
                int(right_ear.y * frame_height),
            )
            ...
            if (
                nose.visibility > self.confidence
                and left_ear.visibility > self.confidence * 0.7
                and right_ear.visibility > self.confidence * 0.7
            ):
                head_detected = True
...
def process_video(video_path, output_path=None, show_preview=True, log_dir="logs", detector=None):
    ...
    try:
        while cap.isOpened():
            ...
            # 骨格検出の実行
            start_time = time.time()
            head_detected, landmarks, display_frame, head_turning = detector.detect_pose(
                frame, frame_count - 1, show_preview
            )
            ...
            if head_turning and current_turning_event is None:
                current_turning_event = {"start_frame": frame_count, "start_time": time.time()}
    ...
    if turning_events:
        ...
        for i, event in enumerate(turning_events):
            print(
                f"イベント{i + 1}: フレーム {event['start_frame']} - {event['end_frame']} "
