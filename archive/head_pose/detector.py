    ...
    def _detect_head_turning(self):
        ...
        if len(self.yaw_history) >= 5:
            ...
            try:
                ...
                # より高度な首振り検出ロジック
                # 1. 連続した閾値超えを検出
                num_significant_changes = sum(
                    1 for v in yaw_velocity[-self.consecutive_frames - 2 : -2] if abs(v) > self.yaw_threshold
                )

                # 2. 方向の変化も検出（左右の振り）
                sign_changes = 0
                for i in range(1, len(yaw_velocity)):
                    if (yaw_velocity[i - 1] > 0 and yaw_velocity[i] < 0) or (
                        yaw_velocity[i - 1] < 0 and yaw_velocity[i] > 0
                    ):
                        sign_changes += 1
