...
from result_loader import ResultLoader, DetectionResult
...
class DetectionEvaluator:
    ...
    def __init__(self, ground_truth_csv: str, video_path: str = None):
        ...
        if video_path and Path(video_path).exists():
            ...
            self.fps = 30.0  # デフォルト
            print(f"⚠️  動画ファイルが見つからないため、推定値を使用します")
            print(f"   総フレーム数: {self.total_frames}, FPS: {self.fps}")
        
        self.calculator = MetricsCalculator()
    
    def evaluate_method(self, predictions: list, method_name: str, 
        ...
        for iou_threshold in iou_thresholds:
            ...
            self.calculator.print_metrics(
                event_metrics, 
                f"🎯 Event-Level Metrics (IoU >= {iou_threshold})"
            )
        
        # 時間的重なりの統計
        overlap_stats = self.calculator.calculate_temporal_overlap(
            self.ground_truth, predictions, event_type
        )
    ...
    def compare_methods(self, mediapipe_csv: str = None, yolo11_csv: str = None,
                       event_type: str = None) -> dict:
        """
        複数の検出手法を比較
        ...
        if mediapipe_csv and Path(mediapipe_csv).exists():
            mediapipe_predictions = self.loader.load_mediapipe_results(
                mediapipe_csv, self.fps
            )
            results['mediapipe'] = self.evaluate_method(
                mediapipe_predictions, "MediaPipe", event_type
            )
        
        # YOLO11の評価
        if yolo11_csv and Path(yolo11_csv).exists():
            yolo11_predictions = self.loader.load_yolo11_results(
                yolo11_csv, self.fps
            )
            results['yolo11'] = self.evaluate_method(
                yolo11_predictions, "YOLO11", event_type
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="検出結果を評価し、比較レポートを生成します"
    )
    parser.add_argument(
        "ground_truth",
        help="正解ラベルのCSVファイル"
    ...
    parser.add_argument(
        "--video",
        help="動画ファイルパス（総フレーム数とFPSを取得）"
    )
    parser.add_argument(
        "--mediapipe",
        help="MediaPipeの検出結果CSVファイル"
    )
    parser.add_argument(
        "--yolo11",
        help="YOLO11の検出結果CSVファイル"
    )
    parser.add_argument(
        "--event-type",
        help="評価対象のイベントタイプ（例: HEAD_SHAKE）"
    )
    parser.add_argument(
        "--output", "-o",
        default="evaluation_report.md",
        help="出力レポートファイル（Markdown形式）"
    )
    ...
    results = evaluator.compare_methods(
        mediapipe_csv=args.mediapipe,
        yolo11_csv=args.yolo11,
