...
class DetectionResult:
    ...
    def __init__(self, start_frame: int, end_frame: int, start_time: float, 
                 end_time: float, event_type: str, confidence: Optional[float] = None):
    ...
    def __repr__(self):
        return (f"DetectionResult(frames={self.start_frame}-{self.end_frame}, "
...
class ResultLoader:
    ...
    @staticmethod
    def load_yolo11_results(csv_path: str, fps: float = 30.0, 
                           event_type_column: str = 'idea') -> List[DetectionResult]:
        """
        YOLO11の検出結果CSVを読み込み
    ...
    @staticmethod
    def filter_by_event_type(results: List[DetectionResult], 
                            event_type: str) -> List[DetectionResult]:
        """特定のイベントタイプでフィルタリング"""
