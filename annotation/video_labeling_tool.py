...
class VideoLabelingTool:
    ...
    def _draw_info(self, frame):
        ...
        cv2.putText(frame, f"Time: {self._format_time(current_time)}", 
        ...
        cv2.putText(frame, f"Status: {status}", 
                    (20, 100), font, 0.7, color, 2)
        
        # 記録状態
        if self.recording_head_shake:
            start_time = self.head_shake_start_frame / self.fps
            duration = current_time - start_time
            cv2.putText(frame, f"[REC] HEAD SHAKE - Duration: {duration:.2f}s", 
                        (20, 130), font, 0.8, (0, 0, 255), 2)
        else:
            cv2.putText(frame, "Ready to label (Press '1' for head shake)", 
                        (20, 130), font, 0.6, (200, 200, 200), 1)
        
        # ラベル数
        cv2.putText(frame, f"Total Labels: {len(self.labels)}", 
                    (20, 160), font, 0.7, (255, 255, 0), 2)
    ...
    def _toggle_head_shake(self):
        ...
        if not self.recording_head_shake:
            ...
            print(f"⏹️  首振り記録終了: Frame {self.head_shake_start_frame}-{end_frame} "
...
def main():
    if len(sys.argv) != 3:
        ...
        print()
        print("例:")
        print("  python3 video_labeling_tool.py ../data/raw/国宝さん手をあげる2.mp4 ground_truth/国宝さん手をあげる2_labels.csv")
        sys.exit(1)
